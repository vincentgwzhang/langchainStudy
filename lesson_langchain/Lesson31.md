...
# :books: 结构化输出 - 使用 json 模式输出

操作步骤

+ 使用 json_mode 输出 JSON 格式的数据
+ 编写运行测试程序
...
# result = structured_llm.invoke(prompt.invoke({"question": "告诉我5个Python的特点"}))
# result = structured_llm.invoke(prompt.invoke({"question": "告诉我5个日本旅游的景点"}))
